...
class Paddle:
    ...
    def find_closest_face(self, frame: np.array, face_detection):
        ...
        if (results.detections):
            ...
            for detection in results.detections:
                ...
                distances.append(np.linalg.norm(self.center - midpoint))
    ...
    def update(self, dt: float, capture: cv.VideoCapture, face_detection):
        frame = capture.read()
        grayscale = cv.cvtColor(frame[:, ::-1], cv.COLOR_BGR2GRAY)
        face = self.find_closest_face(grayscale, face_detection)
        ...
        top_left = np.dot(rotation_matrix, np.array([midpoint[0] - self.width / 2, midpoint[1] - self.hieght / 2]))
        ...
        self.vel = (midpoint - self.center) / dt
