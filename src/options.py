...
class Options:
  def __init__(self, screen_size: tuple, y: float, text: str, options: dict, font: Font, font_size:float =20):
  ...
  def update(self, event: pg.Event):
    ...
    return list(filter(lambda option: option["selected"], self.options))
